...
import { ArrowUpCircle, ArrowDownCircle, TrendingUp, AlertCircle, ChevronRight, Activity, CalendarDays, Edit2, TrendingDown, Info, Wallet } from 'lucide-react';
...
interface DashboardProps {
  ...
  onNavigate: (view: string) => void; // Navegação mantida!
  refreshKey?: number;
}

const InfoTooltip = ({ text, dark = false }: { text: string, dark?: boolean }) => (
  <div className="group/info relative ml-1.5 inline-flex items-center justify-center z-20">
    <Info 
      size={14} 
      className={`${dark ? 'text-white/60 hover:text-white' : 'text-gray-400 hover:text-brand-500'} cursor-help transition-colors`} 
    />
...
const Dashboard: React.FC<DashboardProps> = ({ onEditTransaction, onNavigate, refreshKey }) => {
  const [balanceData, setBalanceData] = useState<DashboardData>({
    realBalance: 0, projectedBalance: 0, monthlyIncome: 0, monthlyExpense: 0
  });
    ...
    const loadDashboardData = async () => {
      try {
        const [balances, allTransactions] = await Promise.all([
           calculateBalances().catch(() => ({ realBalance: 0, projectedBalance: 0, monthlyIncome: 0, monthlyExpense: 0 })), 
           getTransactions().catch(() => [])
        ]);
        
        setBalanceData(balances || { realBalance: 0, projectedBalance: 0, monthlyIncome: 0, monthlyExpense: 0 });
  ...
  const chartData = [
    { name: 'S1', val: 400 }, { name: 'S2', val: 300 }, { name: 'S3', val: 550 }, { name: 'S4', val: 480 },
  ];
  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">Visão Geral</h1>
      ...
      {/* Main Cards Grid - COM SOMBRAS E BORDAS COLORIDAS */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        
        {/* Card 1: Saldo Real (Escuro com Sombra Forte) */}
        <div className="bg-gradient-to-br from-brand-600 to-brand-800 rounded-3xl p-6 text-white shadow-2xl shadow-brand-900/30 relative overflow-hidden group border border-brand-500/30 transform hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
               ...
               <span className="text-xs font-bold bg-emerald-500/20 text-emerald-100 px-2 py-1 rounded-md border border-emerald-500/30">Principal</span>
            </div>
            <p className="text-brand-100 text-sm font-medium mb-1 flex items-center">
               Saldo Atual
               <InfoTooltip text="Soma de todas as contas (Corrente, Dinheiro) exceto crédito." dark />
            </p>
            <h2 className="text-3xl font-bold tracking-tight mb-4 drop-shadow-sm">
               R$ {balanceData?.realBalance?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}
            </h2>
            <div className="flex items-center text-xs text-brand-50 bg-white/10 w-fit px-3 py-1.5 rounded-lg backdrop-blur-md border border-white/5">
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200 border-t-4 border-t-purple-500 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all relative overflow-hidden">
          ...
          <p className="text-gray-500 text-sm font-medium mb-1 flex items-center">
             Saldo Projetado
             <InfoTooltip text="Saldo atual subtraindo faturas de cartão em aberto." />
          </p>
          <h2 className={`text-2xl font-bold tracking-tight mb-2 ${balanceData?.projectedBalance < 0 ? 'text-red-600' : 'text-gray-900'}`}>
             R$ {balanceData?.projectedBalance?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}
          </h2>
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200 border-t-4 border-t-emerald-500 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all">
           <div className="flex items-center justify-between mb-4">
             ...
             <span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100">Mês Atual</span>
           </div>
           <p className="text-gray-500 text-sm font-medium mb-1">Receitas</p>
           <h2 className="text-2xl font-bold text-gray-900 tracking-tight">
              R$ {balanceData?.monthlyIncome?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}
           </h2>
        ...
        {/* Card 4: Despesas (Borda Vermelha) */}
        <div className="bg-white rounded-3xl p-6 border border-gray-200 border-t-4 border-t-red-500 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all">
           ...
           <h2 className="text-2xl font-bold text-gray-900 tracking-tight">
              R$ {balanceData?.monthlyExpense?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}
           </h2>
      ...
      {/* Lista e Acesso Rápido */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Lista de Transações - Sombra Média */}
        <div className="lg:col-span-2 bg-white rounded-3xl border border-gray-200 shadow-lg p-6">
          ...
          <div className="space-y-3">
            {loading ? (
               ...
               <p className="text-center text-gray-400 py-4 italic">Nenhuma transação recente.</p>
            ) : (
               recentTransactions.map((tx) => (
               <div key={tx.id} className="group flex items-center justify-between p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-md hover:border-brand-100 transition-all cursor-pointer">
                  <div className="flex items-center space-x-4">
                     ...
                     <div>
                        ...
                        <p className="text-xs text-gray-500 font-medium mt-0.5 flex items-center">
                           <span className="bg-white border border-gray-200 px-1.5 py-0.5 rounded text-gray-600 mr-2">{format(new Date(tx.date), 'dd MMM')}</span>
        ...
        {/* Acesso Rápido - Sombra Média e Cards Internos */}
...
// Wrappers de Ícones
const WalletIcon = (props: any) => <WalletIconSVG {...props} />;
const CreditCardIcon = (props: any) => <CreditCardIconSVG {...props} />;
