...
import { TrendingUp, TrendingDown, Download, FileText, ArrowUpCircle, ArrowDownCircle, Wallet, CalendarRange } from 'lucide-react';
    ...
    <div className="space-y-8 animate-fade-in-up">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        ...
        <div className="flex space-x-2">
           <button onClick={handleExportCSV} className="flex items-center space-x-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium hover:bg-gray-50 text-gray-700 transition-colors">
             <Download size={16} /> <span>CSV</span>
           </button>
           <button onClick={handleExportPDF} className="flex items-center space-x-2 px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700 shadow-sm transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-brand-200 transition-all">
           ...
           <p className={`text-2xl font-bold ${balance >= 0 ? 'text-gray-800' : 'text-red-600'}`}>R$ {balance.toLocaleString('pt-BR', {compactDisplay: "short", maximumFractionDigits: 0})}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-brand-200 transition-all">
           <div className="flex items-center space-x-3 mb-2">
              <div className="p-2 bg-red-100 rounded-lg text-red-600"><ArrowDownCircle size={18} /></div>
              <span className="text-xs font-bold text-gray-400 uppercase">Maior Gasto</span>
           </div>
           <p className="text-lg font-bold text-gray-800 truncate" title={topCategory}>{topCategory}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-brand-200 transition-all">
           <div className="flex items-center space-x-3 mb-2">
              <div className="p-2 bg-purple-100 rounded-lg text-purple-600"><CalendarRange size={18} /></div>
